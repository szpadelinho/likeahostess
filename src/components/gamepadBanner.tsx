...
const GamepadBanner = ({bannerVisible}: Props) => {
    ...
    const [isGamePad, setIsGamePad] = useState<boolean>(true)
    ...
    return (
        <>
            ...
            <div
                className={`z-[999] flex h-screen w-screen items-center justify-center absolute transition duration-1000 transform ease-in-out ${isGamePad && bannerVisible ? "opacity-100" : "opacity-0"}`}>
            ...
            <div
                className={`z-[999] flex h-screen w-screen items-center justify-center absolute transition duration-1000 transform ease-in-out ${!isGamePad && bannerVisible ? "opacity-100" : "opacity-0"}`}>
                <div className={"flex flex-col h-175 justify-between items-center"}>
                    <Fullscreen size={424} color={"white"}/>
                    <h1 className={`${yesteryear.className} text-[80px] text-white`}>Real hosts use fullscreen mode</h1>
                </div>
            </div>
        </>
    )

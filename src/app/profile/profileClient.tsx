...
const ProfileClient = ({totals, favClub, user, isMe}: ProfileClientProps) => {
    ...
    const [isModal, setModal] = useState<"Reset" | "Delete" | null>(null)
    ...
    const renderModal = () => {
        ...
        const option1 = isModal === "Reset" ? "Please reset my account" : "Please delete my account"
        const option2 = isModal === "Reset" ? "Keep my records" : "Keep my account"
        const action = isModal === "Reset" ? handleReset : handleDelete

        return (
            <div
                onClick={() => setModal(null)}
                className={`${isModal ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"} text-red-600 ${cookie.className} absolute inset-0 duration-300 ease-in-out flex justify-center items-center backdrop-blur-[5px] z-[100]`}>
                <div
                    onClick={(e) => e.stopPropagation()}
                    className={"flex justify-center flex-col gap-5 p-5 items-center bg-[url(/images/paper_texture.png)]"}>
                    <p className={"font-[700] text-[40px] text-center"}>{title}</p>
            ...
            <div className={"grayscale-100"}>
                ...
                <div className={"h-screen w-screen flex items-center justify-center z-50 text-black"}>
                    ...
                    <div className={"absolute top-35 flex items-center justify-center z-50 flex-row gap-10"}>
                        <Image key={avatarSrc} src={avatarSrc || "/images/dragon.png"} alt={"Profile picture"} height={64}

...
const LoginClient = () => {
    ...
    return(
        <>
            ...
            <Navbar router={router} isPlaying={isPlaying} setIsPlaying={setIsPlaying} page={"Login"} volume={volume} setVolume={setVolume} setQuit={setQuit}/>
            ...
            <div className={`${courierPrime.className} absolute inset-0 flex flex-col justify-center items-center text-center gap-5 opacity-70`}>
                ...
                <input className={"border-1 border-black p-1 w-75 ease-in-out duration-300 hover:opacity-100 opacity-70"} type={"text"} placeholder={"Your username or email address"} onChange={(e) => setIdentifier(e.target.value)}/>
                ...
                <button onClick={handleLogin} className={"border-1 border-black p-1 ease-in-out duration-300 hover:opacity-100 opacity-70"}>
                    Log into the system
                </button>
